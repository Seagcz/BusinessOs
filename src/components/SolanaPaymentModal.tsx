...
import { solanaService, DEFAULT_NGN_USDC_RATE, SOLANA_MAINNET_USDC_MINT } from '../services/solanaService';
...
export const SolanaPaymentModal: React.FC<SolanaPaymentModalProps> = ({
  ...
  const handleConnectBrowserWallet = async () => {
    ...
    try {
      ...
      if (wallet) {
        setConnectedPayer(wallet.address);
      } else {
        alert('No Solana wallet extension (Phantom, Solflare, etc.) was detected. Please copy the address or scan the QR code.');
      }
    } catch (err: any) {
  ...
  const handleVerifySignature = async () => {
    if (!signatureInput.trim()) {
      setVerificationError('Please paste the Solana transaction signature.');
      return;
    }
    ...
    try {
      ...
      if (result.verified) {
        ...
        setVerificationError(
          'Transaction was found but is still confirming across Solana Mainnet nodes. Please wait a few seconds and tap Verify again.'
        );
      } else {
        setVerificationError(
          result.error || 'Transaction verification failed on-chain. Please verify the signature and network (Mainnet).'
        );
      }
    } catch (err: any) {
          ...
          {verifiedSuccess ? (
            ...
            <div className="p-6 bg-emerald-950/40 border border-emerald-500/40 rounded-2xl text-center space-y-3 animate-in fade-in">
              ...
              <p className="text-xs text-slate-300 max-w-sm mx-auto">
                Transaction signature successfully confirmed on Solana Mainnet. Payment record and invoice have been settled.
              </p>
              {verifiedDetails?.slot && (
                <div className="p-2.5 bg-slate-950/60 rounded-xl border border-slate-800 text-[11px] text-slate-400 font-mono">
                  Solana Slot: #{verifiedDetails.slot} • Finalized
                </div>
              )}
            ...
            <>
              ...
              <div className="flex flex-col sm:flex-row items-center gap-5 p-4 rounded-2xl bg-slate-950/70 border border-slate-800">
                ...
                <div className="space-y-2 flex-1 text-center sm:text-left w-full">
                  <div className="flex items-center justify-center sm:justify-start gap-1.5 text-xs font-bold text-slate-200">
                    ...
                    <span>Scan with Phantom or Solflare</span>
                  </div>
                  <p className="text-[11px] text-slate-400 leading-relaxed">
                    Open any Solana mobile wallet app, tap Scan QR, and approve the USDC transfer to the business wallet.
                  </p>
                  <div className="pt-1 flex flex-wrap items-center justify-center sm:justify-start gap-2">
                    ...
                    >
                      {copiedItem === 'amount' ? (
                        <span className="text-emerald-400">Copied Amount</span>
                      ) : (
              ...
              {/* One-Click Browser Wallet Button */}
              {solanaService.hasBrowserWallet() && (
                <div className="p-3 bg-gradient-to-r from-purple-950/30 to-indigo-950/30 rounded-2xl border border-purple-500/20 flex items-center justify-between">
                  <div className="flex items-center gap-2.5">
                    <Wallet className="w-4 h-4 text-purple-400" />
                    <div>
                      <p className="text-xs font-bold text-slate-200">
                        {connectedPayer
                          ? `Connected: ${solanaService.truncateAddress(connectedPayer, 4)}`
                          : 'Solana Browser Wallet Detected'}
                      </p>
                      <p className="text-[10px] text-slate-400">
                        Connect to auto-link payer address with this transaction
                      </p>
                    </div>
                  </div>
                  <button
                  ...
                  >
                    ...
                    <span>{connectedPayer ? 'Change Wallet' : 'Connect Wallet'}</span>
                  </button>
                </div>
              )}
                ...
                <div className="space-y-2">
                  <div className="relative">
                    <input
                      ...
                      placeholder="Paste 88-char transaction signature (e.g. 5VERv8...)"
